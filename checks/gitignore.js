...
module.exports = function checkGitignore(context) {
  if (context.config?.requireGitignore === false) return;
  ...
  if (!fs.existsSync(gitignorePath)) {
    if (context.fix) {
      ...
      if (!context.silent) console.log("🛠️  .gitignore created");
    } else {
      if (!context.silent) {
  ...
  // .env yok ama .gitignore var
  if (!content.includes(".env")) {
    if (context.fix) {
      fs.appendFileSync(gitignorePath, "\n.env\n");
      if (!context.silent) console.log("🛠️  .env added to .gitignore");
    } else {
      if (!context.silent) {
